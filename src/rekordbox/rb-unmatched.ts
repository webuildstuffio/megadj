...
export interface RbUnmatchedResult {
  ...
  quarantineLog: { from: string; dest: string }[];
...
export async function quarantineUnmatched(
  ...
  const moved: { from: string; dest: string }[] = [];
...
export async function rbUnmatched(
  ...
  let quarantineLog: { from: string; dest: string }[] = [];
  ...
  return {
    ...
    quarantineLog,
    manifestPath,
    appliedMode: Boolean(opts.quarantine),
    ok: true,
  };
}
