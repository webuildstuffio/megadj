...
function serverRoutes(): Set<string> {
  ...
  const apiRoutes = read("src/deck/api/routes.ts");
  ...
  const dispatch = read("src/deck/api/dispatch.ts");
  ...
  const driveRoutes = read("src/deck/drive/routes.ts");
  ...
  const fleet = read("src/deck/fleet/routes.ts");
...
function deckctlTargets(): ClientTarget[] {
  ...
  clientFiles.push("src/deck/mcp/read-tools.ts");
...
describe("api parity census (#249: the client↔server /api contract)", () => {
  ...
  walkWebTargets("src/deck/web", webTargets);
