...
function httpApiRoutes(): string[] {
  ...
  const index = [
    "src/deck/index.ts",
    "src/deck/api/routes.ts",
    "src/deck/api/dispatch.ts",
    "src/deck/drive/routes.ts",
    "src/deck/fleet/routes.ts",
  ]
    .map((f) => read(f).join("\n"))
    .join("\n");
...
describe("surface parity (docs/surface-parity.md)", () => {
  ...
  test("the product tabs exist and are hash-routed (one route per product)", () => {
    ...
    const app = read("src/deck/web/app/App.tsx").join("\n");
    ...
    const products = read("src/deck/web/products/shared/product-meta.tsx").join(
      "\n",
    );
    for (const product of ["drives", "getdat", "fulltags"])
      expect(products, `nav tab for ${product}`).toContain(`id: "${product}"`);
    const router = read("src/deck/web/app/router.ts").join("\n");
  ...
  test("job-kind lists are derived from the shared/types SSOT (no hand twins)", () => {
    ...
    // re-exports it.)
    const types = [
      ...read("src/deck/shared/types.ts"),
      ...read("src/deck/shared/types/jobs.ts"),
    ].join("\n");
    ...
    for (const f of ["src/deck/deckctl.ts", "src/deck/mcp/action-tools.ts"]) {
      const src = read(f).join("\n");
  ...
  test("mutating MCP tools are flagged destructive + interlock-guarded", () => {
    ...
    const src = readFileSync(
      join(ROOT, "src/deck/mcp/action-tools.ts"),
      "utf8",
    );
    for (const tool of [
      "deck_run",
  ...
  test("the help SSOT is reachable from every surface", () => {
    ...
    const ui = ["src/deck/web/ui/Onboard.tsx", "src/deck/web/ui/JobsDock.tsx"]
      .map((f) => readFileSync(join(ROOT, f), "utf8"))
      .join("\n");
    expect(ui).toContain("../../shared/help");
    // server: GET /api/help serves the same content
    const server = ["src/deck/index.ts", "src/deck/api/routes.ts"]
      .map((f) => readFileSync(join(ROOT, f), "utf8"))
      .join("\n");
    ...
    const deckctlHelp = readFileSync(
      join(ROOT, "src/deck/deckctl/help.ts"),
      "utf8",
    );
    expect(deckctlHelp).toContain('../shared/help"');
    // #89 split: the help handler lives in mcp/read-tools.ts
    const mcp = readFileSync(join(ROOT, "src/deck/mcp/read-tools.ts"), "utf8");
