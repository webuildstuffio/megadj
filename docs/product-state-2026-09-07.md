...
(preflight, player-compat verdicts) and its agent surface (26-tool MCP
...
- **Working today:** the dashboard (registry + ghosts, photos, ports,
  ...
  **26-tool MCP server** (18 `deck_*` + 8 `archive_*`, readonly archive
...
done: `bun run mcp` exposes the whole product (26 tools) over stdio
...
| **3 — Agentify** (the O layer) | MCP server, safety rails, weekly loop | **both MCP halves ✅** (26 tools), rails ✅, attribution ✅, notes ✅, prep ✅, plugin ✅ | **O84** inbox-agent · `claude -p` digest cron · (K61 `megadj drop` underneath it) |
...
[roadmap-proposal.md](roadmap-proposal.md) (the three-move
proposal this page re-scores) · [ideas.md](ideas.md) §0 (the
