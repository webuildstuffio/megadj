...
| Set proposal                                      | `megadj setbuild [--preset --minutes --opener --limit --search]` ✅   | `archive_set_build` ✅ (propose-only; `search` A/B param) | Set product (#/set) + saved draft/M3U8 download ✅ (Sequencer row)      | — (same read-only proposal; requested/actual duration stays explicit)         |
