...
an exemption row is a bug; `src/deck/test/surface-parity.test.ts`
fails the build on it.

## Revision history

- rev-53 (2026-09-23): rev-52 root-cause fix — `?format=m3u8` on `/api/archive/megaset-cohorts` used to be silently IGNORED (a query param a route never reads is the parsed-but-dropped class; the caller got a 200 JSON body that only looked like success). Root cause fixed at three layers: (1) NEW seam `src/deck/megaset/m3u.ts` — the ONE M3U8 renderer (format gate + control-char guard + metadata-only skip + counted note + one header set); the single-set route's inline 55-line renderer is deleted, both routes render through the seam. (2) BOTH set-builder routes now gate `?format=` as a CONTRACT param: absent/blank/json → JSON wire, `m3u8` → download, ANYTHING ELSE → 400 (the silent fall-through is retired on both). (3) The cohorts engine carries the built chains + census rows server-side (`chain`/`poolRows` on CohortBuild — stripped before the JSON wire, `steps` stays a count) so the export renders the SAME build without a re-census. The cohorts export is the WHOLE session as ONE playlist: per-family warmup/peak section comments, SHORT arms labeled, skip note counts across all arms. NEW UI: the Cohorts tab gains an "Export session .m3u8 for Rekordbox" link (`cohortsExportHref` echoes the plan's minutes + current families input). Routes/tools counts unchanged (83 routes / 45 tools).
- rev-52 (2026-09-23): #306 scoring-calibration gate — NEW CLI verb: `megadj megaset-calibrate [--preset P] [--minutes N] [--limit N] [--out FILE|-] [--json]` (scores the LIVE archive over every preset × greedy/beam and writes a DATED digest file; before/after any scoring change, diff the two — a moved `chain=` line or changed hop `t=` value is the scoring change made visible; `--out -` streams to stdout; propose-only, reads the archive DB, writes only the digest). Companion CI gate: `src/deck/megaset/megaset-calibration.test.ts` — a fixed 320-track synthetic corpus through the FULL scoring stack reduces to ONE pinned digest (a moved hop flips it; re-blessing must name the scoring commit), plus a meta-test proving the digest detects changes and constant pins for the F1–F4 drift class. 55 → 56 commands; routes/tools/UI unchanged.
- rev-51 (2026-09-23): #295 full integration — megaset-cohorts leaves CLI-only: the plan engine extracts to `src/deck/megaset/cohorts.ts` (surface-neutral: `buildCohortPlan` + `parseCohortFamilies`, pure over an open ArchiveReader), and ALL surfaces wrap it. NEW route: `GET /api/archive/megaset-cohorts?minutes=&families=&limit=` (propose-only; unknown family → 400 with the known list; short arm = 200 with `complete:false` — a result, not an error). NEW MCP tool: `megaset_cohorts {minutes?, families?, limit?}` (aims at the same route). NEW UI: megaset product's **Cohorts** tab (per-family warmup/peak cards with shortfall + blend + same-artist counts, all_complete verdict, outside-scope note). Explicit-but-empty `--families` is now an ERROR on every surface (the CLI previously ran zero cohorts and exited 0). 44 → 45 tools, 82 → 83 routes.
- rev-50 (2026-09-23): #295 megaset-cohorts — NEW CLI verb: `megadj megaset-cohorts [--minutes N] [--limit N] [--families LIST] [--json]` (the genre-cohort builder: warmup + peak chain PER genre family in ONE run; default families = the measured big four edm/house/techno/tech house, `--families` narrows; blank-genre tracks reported outside scope, never guessed; exit 1 when an arm falls short). Reuses the single-set census+engine — no second pool. CLI-only (§4-A1: propose-only, no route/MCP twin — the per-family payloads are derivable from `megadj megaset --genre F`). Also #304: direction-wall tripwire tests pin the S13 repair pass's honest refusal (monotone-preset contract + descending-wall landmarks_missing pin). 54 → 55 commands.
- rev-49 (2026-09-23): #289 analysis catch-up — NEW CLI verb: `megadj catch-up [--limit N] [--jobs N] [--max-seconds S] [--force] [--dry-run] [--json]` (one analysis gap pass for new imports: beats → mood; ledgered == analyzed unless --force, the #278/#279 short-circuit rides through the existing passes; parallelism --jobs 4 lives HERE, never in sync). CLI-only by design (§4-A1: analysis writes stay CLI); the FreshnessLine note now names the one verb. 53 → 54 commands; routes/UI calls unchanged. Shipped with #286 stages_ms (measured per-stage build timings on the megaset wire) and #293 draft load (web builder ingests its own saved draft JSON — parse → hydrate controls → user presses Build).
- rev-48 (2026-09-21): #282 reconcile scoping — `megadj rb-playlist <drive> reconcile` is now scoped to the agent-written groups (DJ-Imports, MegaSets + `--group NAME`) by default; `--all` opts into the legacy whole-DB diff (the mode that false-flagged ~180 RB-managed playlists as missing twins). Wire gains `scopeRoots` + `skippedByScope` (additive). 53 commands unchanged.
- rev-47 (2026-09-20): super-sure fixes on rev-46 — the funnel buttons now WORK and the job legs stop lying. WIRE: `stages.rawBatches[].folder` is now ABSOLUTE (it was a bare batch name, which /intake/start's absolute-path allowlist 403'd — every shipped Process button was dead); button labels show the batch name, POSTs carry the path. `POST /api/archive/surfaced-batch` now enforces the SAME intakeCandidateDirs allowlist as /api/intake/start (403 off-list; closes the crafted-path asymmetry). JOB BEHAVIOR: the ingest audit leg DEGRADES on unparseable child output (`audit:null`, logged; the wire type already allowed it) instead of failing a job whose imports had already landed (live: 42a682db, 30 real rows marked failed); IntakeRun renders "archive audit unavailable". CONFIG: new `uvPath` resolution (MEGADJ_UV_BIN → UV_BIN → ~/.local/bin/uv → bare name) feeds every rb.ts spawn — the launchd server's PATH lacks ~/.local/bin and unattended verify 404'd on bare "uv" (live: 6b37500c). PY READ: `rb_read.py`'s import probe checks the devicelib_plus surface and prints the exact pinned uv invocation on mismatch (the an-ad-hoc-uv ModuleNotFoundError trap); rb-read tests' two unpinned spawns now use the production pin (fork vs PyPI have DISJOINT APIs — devicelib_plus exists only on the fork, db6 deprecation shim only there too; the rb-command-kit's plain default is correct for db6-style scripts and must not flip). Route/tool/command counts unchanged. 53 commands, 82 routes, 65 UI calls.
- rev-46 (2026-09-19): #260 processing funnel + job-based surfaced batch — "downloaded ≠ finished" becomes visible. WIRE: `GET /api/archive/analysis-coverage` gains optional `stages` {noGenre, genreVoted, rawBatches[]} (column-guarded — pre-voting DBs omit it). UI: Pipeline tab's EnrichmentFunnel decomposes the playable pool (genre-voted meter, per-batch one-click Process buttons → the REAL intake job + auto-nav to the Intake tab, genre-pass jump to FullTags ⌗ Run); SurfacedLinksCard's batch button now enqueues the intake job (jobId returned, 30s deadline — the 600s blocking spawn is retired) and navigates to the Intake tab. Route/tool/command counts unchanged (stages rides an existing route; the batch still POSTs `surfaced-batch`). 53 commands, 82 routes, 65 UI calls.
...
`src/deck/test/surface-parity.test.ts`; the current census is in §1 and the
...
| ---------- | ----------------------------------------------------------------- | ---------------------- |
| megadj CLI | `megadj <cmd>` (`src/cli.ts`)                                     | 56 commands + `--help` |
| deckctl    | `bun run src/deck/deckctl.ts <verb>`                              | 24 verbs               |
| MCP        | `bun run mcp` (`mcp.ts` + `archive/tools.ts` + `getdat_tools.ts`) | 45 tools               |
| HTTP API   | `src/deck/index.ts` + `api_routes.ts` (localhost:7742)            | 83 routes              |
| Web UI     | `src/deck/web/` (hash-routed pages)                               | 6 pages, 66 UI calls   |
...
| catch-up (beats+mood gap pass, #289)              | `megadj catch-up` ✅ — one pass for new imports; ledgered == analyzed unless --force                                                                                                                                 | ⛔ §4-A1 (analysis writes stay CLI)                                                                                  | FreshnessLine names the verb                                                                                        | —                                                                             |
| organize                                          | ✅                                                                                                                                                                                                                   | ⛔ §4-A1                                                                                                             | ⛔ §4-A1                                                                                                            | —                                                                             |
...
| MegaSet proposal                                  | `megadj megaset [--preset --minutes --opener --limit --search]` ✅ (`setbuild` alias retired Sep 15 2026) + `megadj megaset-cohorts` ✅ (#295: warmup+peak per family, one run)                                      | `megaset_propose` ✅ (propose-only; `search` A/B param) + `megaset_cohorts` ✅ (rev-51: the per-family session plan) | MegaSet product (#/megaset): Build tab + **Cohorts** tab ✅ (rev-51) + saved draft/M3U8 download ✅ (Sequencer row) + session .m3u8 export ✅ (rev-53) | — (same read-only proposal; requested/actual duration stays explicit)         |
...
| RB playlist reconcile (XML twin)                  | `megadj rb-playlist <drive> reconcile [--group NAME] [--all] [--apply --yes]` ✅ (#282: scoped to agent-written groups — DJ-Imports, MegaSets — by default; --all = legacy whole-DB diff)                            | ⛔ §4-A1                                                                                                             | ⛔ §4-A1                                                                                                            | —                                                                             |
...
- **L1 — cross-tier twin registry (the "same job, two packages" list).**
  ...
  `src/deck/db/core.ts` DBCore vs `src/shared/sqlite-ledger.ts`
  `openLedger()` (both open WAL + busy_timeout 5000 + synchronous NORMAL;
  aligned by convention Sep 18 per #87's ride-along — never a
  cross-package import; `src/deck/shared/types.ts` stays the import
...
1. **`src/deck/test/surface-parity.test.ts` (shipped with this doc).**
...
2. **`src/deck/api/api-parity-census.test.ts` (#249, shipped
   ...
   client leg walks `src/deck/web` + the deckctl/MCP legs (including
