...
| Similar tracks (I49 sounds-like)                  | `megadj similar <id>` ✅                                                                        | `archive_similar_tracks` ✅                                      | FullTags ⌗ Similar ✅                                                                   | —                                                                    |
| MegaSet proposal                                  | `megadj setbuild [--preset --minutes --opener --limit --search]` ✅ | `archive_set_build` ✅ (propose-only; `search` A/B param) | FullTags ⌗ Similar panel + saved draft/M3U8 download ✅ (Sequencer row) | — (same read-only proposal; requested/actual duration stays explicit)         |
