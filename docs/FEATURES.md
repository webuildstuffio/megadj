...
| **Sources today**  | YouTube Music (liked songs, playlists); **SoundCloud downloads (Sep 19, [#255](https://github.com/webuildstuffio/megadj/issues/255))** — `sync --sc-url <track-or-set>`, `--sources sc-user:NAME,sc-likes:NAME`, **link-first** ([#256](https://github.com/webuildstuffio/megadj/issues/256)): an official purchase/free-download link is offered to the user instead of ripping (`--force-rip` overrides); rips at SC's 160k-AAC ceiling; SoundCloud + Beatport + Bandcamp still vote genre/year/label/art at `megadj fetch` time; the **imprint prior (Sep 16, W7)** converts Beatport-filled labels into a genre vote when the catalogs miss |
| **Sources coming** | 1001tracklists mining as a discovery queue (#110) · Bandcamp downloads once yt-dlp's extractor recovers (#124 context) · SoundCloud-as-download-source shipped from the [plan PRD](getdat/soundcloud-downloads-plan.md) (#109's re-file)                                                                                                                                                                                                                                                                                                                                                                                                        |
| **How it works**   | `megadj sync` → yt-dlp at the best format available per source (YT: 256 kbps AAC first; SC: hls_aac_160k, the platform ceiling); polite pacing and backoff; permanent failures classified and never retried (dead SC permalinks = gone, DRM/Go+ = permanent)                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **Flag**           | anything below the source's floor is flagged `LOWQ` in `megadj list` — YT: 250 kbps; SC: its own 160k ceiling (#258), so SC rips aren't noise-flagged — quality only ratchets up. SC downloads map the payload's `timestamp` onto date (SC has no `release_date` — measured Sep 19); #275: the artist is the title's "Artist - Track" split when the uploading channel differs (label/imprint pages uploaded most paro tracks — the channel is provenance, never the artist), and fetch's Beatport stage self-heals legacy rows by re-deriving the artist the same way                                                                          |
