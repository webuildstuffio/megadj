...
|                    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Status**         | 🔨 YouTube Music + SoundCloud downloads today; Beatport/Bandcamp enrichment live in the fetch ladder                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Sources today**  | YouTube Music (liked songs, playlists); **SoundCloud downloads (Sep 19, [#255](https://github.com/webuildstuffio/megadj/issues/255))** — `sync --sc-url <track-or-set>`, `--sources sc-user:NAME,sc-likes:NAME`, **link-first** ([#256](https://github.com/webuildstuffio/megadj/issues/256)): an official purchase/free-download link is offered to the user instead of ripping (`--force-rip` overrides); rips at SC's ceiling — **256k AAC first (Plus session, since Sep 19)**, 160k anonymous; SoundCloud + Beatport + Bandcamp still vote genre/year/label/art at `megadj fetch` time; the **imprint prior (Sep 16, W7)** converts Beatport-filled labels into a genre vote when the catalogs miss |
| **Sources coming** | 1001tracklists mining and Bandcamp-as-download-source were both closed NOT_PLANNED (2026-09-19) — see #110/#124; SC-as-download-source SHIPPED from the [plan PRD](getdat/soundcloud-downloads-plan.md) (#109's re-file)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **How it works**   | `megadj sync` → yt-dlp at the best format available per source (YT: 256 kbps AAC first; SC: hls_aac_256k with a Plus session, hls_aac_160k anonymous — the anonymous ceiling); polite pacing and backoff; permanent failures classified and never retried (dead SC permalinks = gone, DRM/Go+ = permanent)                                                                                                                                                                                                                                                                                                                                                                                               |
| **State**          | SQLite tracks every ID (YT video ids and numeric SC track ids — disjoint shapes): status, format, bitrate, path, attempt history, and surfaced links. Nothing re-downloads; a surfaced link is never counted as downloaded. `drop`'s SC link-surface now LEDGERS the decision (numeric SC id, `link_surfaced` + `source_links`) so `sync` never re-attempts it; `megadj status` reports the cohort (`links_surfaced`) and `megadj list` renders it as `LINK` with the URL                                                                                                                                                                                                                                |
| **Flag**           | anything below the source's floor is flagged `LOWQ` in `megadj list` — YT: 250 kbps; SC: the SC ceiling itself (160k aac / 128k mp3, #258) so SC rips aren't noise-flagged, while an under-ceiling rip still flags — quality only ratchets up. SC downloads map the payload's `timestamp` onto date (SC has no `release_date` — measured Sep 19); #275: the artist is the title's "Artist - Track" split when the uploading channel differs (label/imprint pages uploaded most paro tracks — the channel is provenance, never the artist), and fetch's Beatport stage self-heals legacy rows by re-deriving the artist the same way                                                                      |
...
|                   |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Status**        | ✅ graduated product — v0 shipped propose-only (Sep 11–12 2026): greedy Camelot/energy-arc engine, whole-library pool, CLI + web + MCP + M3U8 export, gated `megadj rb-playlist` write-off; own doc set since Sep 14. v1 = the [re-ranked roadmap](megaset/03-competitive-analysis.md). Sep 16: #106 Phase D cue-window handoffs + #171 embeddings similarity prior. Sep 20 (#283): `--genre` pool filter (family matcher + starvation fallback) on every surface; round-2 pass ships `avg_transition`/`min_transition` on the wire, un-noops `rb-playlist --search`, and lands dedupe v2 after the 3×-Epoch incident — [session report](megaset/2026-09-20-megaset-session-five-sets.md). Sep 21 (audit pass): dedupe v3 (bare release forms, head-credit artists, in-title extensions — three live dupes fixed), exclusion reasons grouped by CLASS (130 buckets → 3), transition bands recalibrated for the bonus-era score, dated drafts with request context, `--genre` visible in the CLI header and the web repro line — [audit + next](megaset/2026-09-21-audit-and-next.md). Sep 21 (improvement pass): **B6 diversity guard** (same head-credit artist back-to-back ranks last via `megasetArtistRepeatPenalty`, `same_artist_pairs` on the wire), **B8 half-time lane** (pairings near ×2/×½/×1.5/×⅔ score 0.75 instead of 0 — DnB closers on an 87 anchor now land), **S13 landmark pins** (`--landmark <id>` repeatable on CLI/web/MCP; the engine inserts each pin at its first arc-legal position and reports unplaceable ones in `landmarks_missing`), and dedupe v4 (uploader-channel fold — "Trap City" vs "Trap Nation" for the same self-titled remix is one recording). Live proof: 90-min peak house (15 tracks, avg 1.131, zero same-artist pairs), 60-min afterhours (18 tracks), 75-min peak house with pins (pin landed at 23.7m, ghost pin honestly reported). Sep 21 (queue burn-down, `23379c2c`): **#284 per-step evidence** (`evidence` on every step — weight-scaled tempo/key/arc/anchor/similarity + B6 penalty + B8 flag, `total` == blend, rendered in CLI/web/MCP), **#291 budget-fill as status** (`budget_filled` count; `excluded_groups` quality-only), **#290 genre guard** (`genre_suggestion` nearest-family hint on 0-row filters + the fallback-widening bug fix: resolution is by family, not raw string — `--genre tropical` now widens identically to `--genre "tropical house"`), **#294 inline score magnitude** in the web mix pill — [full report](megaset/2026-09-21-full-report.md) §0. |
