...
| **State**          | SQLite tracks every ID (YT video ids and numeric SC track ids — disjoint shapes): status, format, bitrate, path, attempt history, and surfaced links. Nothing re-downloads; a surfaced link is never counted as downloaded.                          |
| **Flag**           | anything below the source's floor is flagged `LOWQ` in `megadj list` — YT: 250 kbps; SC: its own 160k ceiling (#258), so SC rips aren't noise-flagged — quality only ever ratchets up                                                                 |
