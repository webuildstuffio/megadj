...
| **Flag**           | anything below the source's floor is flagged `LOWQ` in `megadj list` — YT: 250 kbps; SC: its own 160k ceiling (#258), so SC rips aren't noise-flagged — quality only ever ratchets up. SC downloads map the payload's `uploader`/`timestamp` onto artist/date (SC has no `artist`/`release_date` fields — measured Sep 19)                                                                                                                                                                                                                                                                                                                      |
