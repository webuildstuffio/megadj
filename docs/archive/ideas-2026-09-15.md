...
> **Status:** 🗄️ ARCHIVED (was 🧭 ACTIVE) — ordered backlog; GitHub issues are the execution tracker.
...
6. **Coverage matrix — ✅ SHIPPED 2026-09-04.** `src/deck/coverage.ts` + `deckctl coverage`.
...
75. **Hardware compatibility matrix as data (Device vs OneLibrary) — ✅ SHIPPED 2026-09-05.** `src/deck/players.ts` + `deckctl players` + `deck_players`; verdicts from the drive's MEASURED dual-DB rows, folded into preflight (a fully blocked drive is not-ready).
...
82. **megadj MCP server — ✅ SHIPPED 2026-09-05 (both halves).** Live: `bun run mcp` (deck + getdat tools) + `src/deck/archive_tools.ts` (readonly archive reads).
