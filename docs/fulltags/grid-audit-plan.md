...
| Tempo readouts                        | `src/fulltags/beats-analysis.ts` (`analyzeBeats`, median inter-beat; the since-deleted `tempoFromBeatGrid` bar-lag)               | TBPM tag writes **blocked by gate** (12/24, re-gate 16/24 — the ~2.2–2.6% phase-lock); arrays are DB-only by decision                                                                       |
