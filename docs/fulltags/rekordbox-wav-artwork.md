...
research and the decision; the one-off tool (`rb_art.py`) was deleted
Sep 16 2026 (issue #93 CUT — zero remaining workload; recoverable from
git history).
...
> converts every new WAV to AIFF on the way in** (`src/fulltags/write/convert.ts`),
...
**Tool:** `rb_art.py` (deleted Sep 16 2026 — issue #93 CUT: the WAV→AIFF
intake converter left it zero-workload and the batch completed Sep 4.
Recoverable from git history; historical usage preserved below for the
record. Modes ran `status` / `dry-run` / `pilot` / `batch`):
...
uv run --with "pyrekordbox @ git+https://github.com/dylanljones/pyrekordbox.git@f695541827cc488af267d6ca8a8e0052598d85a0" \
    --with mutagen --with Pillow python tools/rb_art.py  status   # deleted tool — historical

# 1. dry-run — plan every write, touch nothing
uv run --with "pyrekordbox @ git+https://github.com/dylanljones/pyrekordbox.git@f695541827cc488af267d6ca8a8e0052598d85a0" \
    --with mutagen --with Pillow python tools/rb_art.py  dry-run # deleted tool — historical

# 2. pilot — write 3 tracks only, then YOU open rekordbox and verify covers show
uv run --with "pyrekordbox @ git+https://github.com/dylanljones/pyrekordbox.git@f695541827cc488af267d6ca8a8e0052598d85a0" \
    --with mutagen --with Pillow python tools/rb_art.py  pilot   # deleted tool — historical

# 3. batch — all remaining WAVs
uv run --with "pyrekordbox @ git+https://github.com/dylanljones/pyrekordbox.git@f695541827cc488af267d6ca8a8e0052598d85a0" \
    --with mutagen --with Pillow python tools/rb_art.py  batch   # deleted tool — historical
```
...
covers). The tool is gone; if a legacy-WAV edge case ever resurfaces,
recover `rb_art.py` from git history and re-add it to the python gates.
