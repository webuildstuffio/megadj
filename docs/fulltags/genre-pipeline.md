...
| Run-tab server side: `fetch` job kind, feed ring, `/api/fetch/start` + `/api/fetch/feed` (#215)                                                    | `src/deck/jobs/job-legs-fetch.ts` (`runFetchJob`, re-exported by `job-legs.ts`) + `src/deck/fetch-feed.ts` + `src/deck/api/routes.ts` (`fetchRoutes`)                    |
...
| Tag census + three-source compare (T1/T2)                                                                                                          | `src/deck/archive/tag-census.ts`                                                                                                                                                   |
