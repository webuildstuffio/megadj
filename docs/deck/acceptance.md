...
here is **code-verified only** (file/route/test existence in `src/deck/`).
...
- **F1 registry & ghosts** — `src/deck/registry.ts`, `src/deck/db.ts` (drives/events/
  snapshots tables), ghost rendering in `src/deck/web/app/App.tsx`
...
- **F10 extras** — dossier export (`GET /drives/:id/export` incl. report),
  deckctl CLI (`src/deck/deckctl.ts`, agent-facing with interlock
...
- **F10 radar (SHIPPED 2026-09-16, #148)** — archive-vs-drive delta over
  one pure engine (`src/deck/fleet/radar.ts`, folded-path + artist-title
...
(`src/deck/web/app/router.ts`). Redesigned: SVG icon set (`src/deck/web/ui/icons.tsx`), design
tokens (`src/deck/web/styles/`), drive rail cards (health ring, role chips, space bar,
...
- [x] Build plan M6: SIGKILL recovery is regression-tested against a real
      ...
      write path and database reopen (`src/deck/test/issue-33-crash-recovery.test.ts`).
