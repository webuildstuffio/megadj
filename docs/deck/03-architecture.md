...
```
src/deck/
  src-less root   Bun + TS — the whole server (domain subfolders since #214:
...
Python owns _rekordbox truth_ — through a single seam (`src/deck/rb.ts` ↔
`src/deck/python/rb_read.py` / the skill's `usb_verify.py` / `usb_mirror.py`). The
...
## 4. The Python seam (`src/deck/rb.ts` → `python/`)

**Reads** (scan-level): copy `exportLibrary.db` (+wal/shm) to
`src/deck/data/scratch/`, then `uv run python python/rb_read.py <copy>` → one JSON:
...
Preact + Vite. `src/deck/web/app/` owns composition and the zero-dependency hash router;
`src/deck/web/products/` owns the CrateDeck, GetDat, FullTags, MegaSet, and shared
canvases (MegaSet's page rides `#/set` with the form/proposal panel in
`src/deck/web/products/fulltags/MegasetPanel.tsx`); `src/deck/web/ui/` owns
shared components; `src/deck/web/styles/` owns per-concern stylesheets. Deep links and
browser back/forward work without a router dependency. SSE reconnects and
coalesces job updates. Badge rules live in `src/deck/shared/badges.ts`, computed
...
| SQLite corruption              | WAL + nightly `src/deck/data/backup.sqlite`; images relink by UUID |
