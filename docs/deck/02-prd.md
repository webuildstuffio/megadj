...
`src/deck/` (registry, detection, images, rekordbox introspection via the
...
1. **Product image search** — type a model, get an image grid. Provider
   ...
   active provider + API key live in `src/deck/config.sample.toml` (copied
...
`src/deck/data/images/<uuid>/`, and never re-fetched.
