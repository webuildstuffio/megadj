...
_(Private telemetry stays off — but the [co-occurrence lane](04-sequencing-benchmarks.md#part-5)
...
| 11  | **Embedding similarity prior** (B10p)                      | Unique data among all 30; keep the weight small and flag-gated until it proves out in A/B proposals. Extended by the [co-occurrence lane](04-sequencing-benchmarks.md#part-5) — public tracklists (1001TL 260k+, MixesDB API) as a second, independent scene-affinity signal; useful from ~10k matched rows, pair-level steering deliberately never built | B                | S    |
