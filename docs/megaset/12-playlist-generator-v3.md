...
It IS a re-architecture of everything _around_ the engine: what the generator
...
| Tempo anchor + drift budget                                                                                                   | **Shipped** (B2): ±12% anchor budget, half/double-time branch lane at ±6% (B8 partially in: the _branch lane_ exists in `withinAnchorBudget`, but `bpmScore` itself still scores 0 for a raw 87↔174 pair outside the branch window)                                                                                                                                                                                                                                                    | `megaset/scoring.ts`                                                                                              |
...
_absorb_ those, not re-propose them.
...
1. **The generator is order-only.** It outputs an ordered list with per-step
   cue windows, but has no model of the _transition itself_ — type (long
...
— all _sequencing decisions that don't need track data_. The fill stays the
...
This is scoped to stay propose-only: types are _labels on windows_, not
...
1. **No transition _synthesis/rendering_** — types describe windows; no audio
...
| 3   | `buildPlan()` extraction (M3) — refactor-only, byte-identical-chain pinned; landmarks become plan inputs | [#312](https://github.com/webuildstuffio/megadj/issues/327)                       | **SHIPPED Sep 23** (`plan.ts`: buildPlan + serializable MegasetPlan, golden-chain pinned in engine-plan.test.ts; 02-architecture §2 P1–P8 rows) |
...
_shape_ (typed windows) with the grammar, and extends its build order — it
