...
**Plan row-set (M3, [#327](https://github.com/webuildstuffio/megadj/issues/327)
shipped Sep 23)** — `buildPlan()` (`megaset/plan.ts`) makes the sequencing
decisions that need no track data a serializable `MegasetPlan` (JSON
round-trips byte-identical, pinned). P-rows are PLACEHOLDERS documenting what
exists; consumers (M4 quality, M5 grammar, M6 sessions) land their own rows:

| #   | Plan field          | Type                                                                | Source                             | Status | Notes                                                                              |
| --- | ------------------- | ------------------------------------------------------------------- | ---------------------------------- | ------ | ---------------------------------------------------------------------------------- |
| P1  | `segments`          | 3 × `{role, window, bpmTarget, keyRegion, energyRange, danceRange}` | preset envelope cut into B3 thirds | ✅ M3  | B3's segmentation first-class; `keyRegion` null until a track anchors (fill-owned) |
| P2  | `opener`            | `{requestedId, minNeighbors, tieBreak, startArousal}`               | S3/S8/S10 constants + request      | ✅ M3  | the fill re-derives the pick; the plan records the policy                          |
| P3  | `driftBudget`       | number (0.12)                                                       | `MEGASET_DRIFT_BUDGET` (B2)        | ✅ M3  | S6's anchor gate as plan data                                                      |
| P4  | `branchLanes`       | `[2, 0.5]`                                                          | B8 half-time lanes                 | ✅ M3  | which drift ratios the budget spares                                               |
| P5  | `landmarkIds`       | `videoId[]` (deduped, request order)                                | S13 `--track` pins                 | ✅ M3  | pins recorded on the Plan; fill still repair-passes (no behavior change)           |
| P6  | `strategy`          | `"beam" \| "greedy"` + `strategyForced`                             | E7 pool-size rule or S12 override  | ✅ M3  | crossover `poolSize < 250` ≡ the engine's `rest.length + 1 <` (boundary-pinned)    |
| P7  | `budgetS`/`minutes` | seconds / int                                                       | S2                                 | ✅ M3  | the fill's clock                                                                   |
| P8  | `version`           | `1`                                                                 | —                                  | ✅ M3  | bump on shape change; sessions (M6) persist this                                   |
