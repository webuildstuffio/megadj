...
```bash
megadj megaset --preset warmup --minutes 45          # human table on stdout
megadj megaset --preset peak --minutes 60 --json     # the full payload
megadj megaset --opener <video_id> --preset afterhours
```
...
- `--limit` candidate pool cap 1–1000 (default unlimited; the clamp is
  shared with the route/MCP contract)
...
- Engine SSOT: `cratedeck/src/megaset.ts`; wire shapes:
...
```jsonc
{
  ...
  "preset": "peak",       // id — resolve labels from MEGASET_PRESET_DEFS
...
real playlist BY HAND — the write-off is `megadj rb-playlist`
(dry-run-first, separately gated), and nothing in this workflow stamps
tags or touches drives.
