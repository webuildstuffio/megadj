...
| MCP       | `.mcp.json` → `cratedeck`   | 43 tools (23 `deck_*` + 2 `getdat_*` + 18 `archive_*`+`megaset_*`): drive health, fleet, radar, preflight, players, notes, rename, dossier export, hygiene + booth fixes, booth fleet, GetDat intake/conversion, search, prep digest, archive reads (grid cross-check, mood profile, sweep, tag census/compare, genre-why, MegaSet proposal) |
