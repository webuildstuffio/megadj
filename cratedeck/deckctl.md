...
| `hygiene [scan\|apply\|confirm ID\|dismiss ID]` | **shelf hygiene queue** (shelf drive only): bare = read-only census; `scan`/`apply` enqueue jobs; `confirm`/`dismiss` decide one finding (the human gate before apply)                               |
