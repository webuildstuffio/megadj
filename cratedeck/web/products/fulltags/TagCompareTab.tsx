    ...
    <Card>
      ...
      <KVRows>
        ...
        <KVRow key="year">
          ...
          <KVVal>
            file {show(t.file?.year ?? null)} / {show(t.file?.label ?? null)} ·
            rb {show(t.rekordbox?.year ?? null)} /{" "}
            {show(t.rekordbox?.label ?? null)}
          </KVVal>
        </KVRow>
        <KVRow key="mood">
          ...
          <KVVal>
            ...
            {t.pipeline.valence !== null &&
              ` · V${t.pipeline.valence.toFixed(1)}`}
            {t.pipeline.arousal !== null &&
              ` · A${t.pipeline.arousal.toFixed(1)}`}
            {t.file?.mood && (
              <span class="dt-sub"> · TXXX:MOOD {t.file.mood}</span>
            )}
          </KVVal>
        </KVRow>
        <KVRow key="art">
