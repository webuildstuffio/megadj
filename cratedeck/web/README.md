...
`bun.lock` in this directory is **intentional**; do not fold `cratedeck/web`
into the root `workspaces` glob.

## Why isolated

- This package is **Preact** (`preact`, `@tanstack/preact-table`,
  `lucide-preact`); the root and `cratedeck/src` tree carries no React at
...
- The web app is built by its own Vite config (`bun run web:build` from
  ...
  `cratedeck/shared/types.ts`, the import leaf — is a **type-only**
