...
- Genre vote ladder (#173) SHIPPED in f54ba04 (Sep 16, closed Sep 17): every rung votes (genre+weight+provenance), one election seam (`stageGenreElection`) owns tag+DB write, breakdown persists in `tracks.genre_votes`, weights versioned in code as `GENRE_VOTE_WEIGHTS` (test-pinned ordering). First-win writes are gone. GAP: the breakdown has a writer but ZERO production readers (`ArchiveState.genreVotes()` uncalled) — tracked as #215; knip is blind to dead methods on live classes, only a caller census catches that class.
