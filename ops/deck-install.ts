 ...
 * installer kills bare `bun ... cratedeck/src/index.ts` processes that
...
import { DEFAULT_PORT, resolveServerPort } from "../cratedeck/src/server-port";
...
function orphanDeckProcesses(): number[] {
  const r = sh(["pgrep", "-f", "cratedeck/src/index.ts"]);
  if (r.code !== 0) return [];
  return r.out
    .split("\n")
    .map((s) => s.trim())
...
async function main(): Promise<void> {
  ...
  const configPath = join(REPO_ROOT, "cratedeck", "config.toml");
  const port = resolveServerPort(
    process.env.CRATEDECK_PORT,
    existsSync(configPath)
      ? configPath
      : join(REPO_ROOT, "cratedeck", "config.sample.toml"),
  );
